...
if 'async def _run_company_workflow(' not in s:
    ...
    company_method = '''    async def _run_company_role_direct(self, chat_id: str, bot_id: str, bot_config: dict, role_key: str, task_text: str) -> None:
    ...
    async def _run_company_workflow(self, chat_id: str, bot_id: str, bot_config: dict, task_text: str) -> None:
...
if 'async def _run_company_role_direct(' not in s:
    ...
    if workflow_marker not in s:
        raise SystemExit('Expected _run_company_workflow marker for direct role method')
    s = s.replace(workflow_marker, direct_role_method + workflow_marker, 1)
    patched = True

# Ensure slash/bare role commands are routed before follow-up handling.
direct_guard = '            direct_match = re.match(r"^\\s*/?(pm|frontend|designer|backend|qa|techlead)\\s+(.+)$", str(task_text or ""), re.IGNORECASE | re.DOTALL)\n            if direct_match:\n                await self._run_company_role_direct(chat_id, bot_id, bot_config, direct_match.group(1).lower(), direct_match.group(2).strip())\n                return\n'
chain_marker = '            chain = bot_config.get("companyWorkflow")\n'
if 'direct_match = re.match' not in s and chain_marker in s:
    s = s.replace(chain_marker, direct_guard + chain_marker, 1)
    patched = True
